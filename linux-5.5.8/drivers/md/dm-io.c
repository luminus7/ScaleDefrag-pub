		   ...
		   unsigned long size)
{
	...
	case DM_IO_PAGE_LIST:
		list_dp_init(dp, io_req->mem.ptr.pl, io_req->mem.offset);  //zgy: mem.ptr.pl point to allocated pages
		break;
	  ...
	  struct dm_io_region *where, unsigned long *sync_error_bits)
{
	...
	r = dp_init(io_req, &dp, (unsigned long)where->count << SECTOR_SHIFT);  // zgy: count has no meaning, it is not used here
	if (r)
		return r;
